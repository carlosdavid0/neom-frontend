...
function OltList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (enabled, olt, i) => (
        <Switch size="small" defaultChecked={enabled} onChange={() => changeEnabled(olt.id)} />
      ),
    },
    {
        ...
        <Space size="middle">
          <Button
            ...
            onClick={() => {
              ...
              FormEdit.setFieldsValue({ superuser: record.superuser });
              FormEdit.setFieldsValue({ superpass: record.superpass });
            }}
          >
            Edit
          </Button>
          <Popconfirm title={t('questions.remove_olt')} onConfirm={() => removeOlt(record.id)}>
            <Button size={'small'} type={'danger'} icon={<CloseSquareOutlined />}>
              Remove
            </Button>
          </Popconfirm>
        </Space>
  ...
  function newOlt() {
    ...
    API.post('olts', {
      ...
      superuser: FormCreate.getFieldValue('superuser'),
  ...
  function editOlt() {
    API.patch(`olts/${FormEdit.getFieldValue('id')}`, {
      ...
      superuser: FormEdit.getFieldValue('superuser'),
  ...
  return (
    <Row>
            ...
            <Table
              ...
              pagination={{
                ...
                showTotal: (total, range) =>
                  `${range[0]}-${range[1]} ${t('texts.of').toLocaleLowerCase()} ${total} ${t(
                    'texts.items'
                  ).toLocaleLowerCase()}`,
      ...
      >
        <Form form={FormCreate} onFinish={newOlt} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
          <Form.Item
            name={'name'}
            label={t('texts.name')}
          ...
          >
            <Select placeholder={t('tables.model')} suffixIcon={<BranchesOutlined />}>
              {vendor === 'Fiberhome' ? (
                <>
      ...
      >
        <Form form={FormEdit} onFinish={editOlt} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
